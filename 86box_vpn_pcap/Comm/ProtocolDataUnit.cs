...
namespace EightSixBoxVPN.Comm
{
    ...
    public class ProtocolDataUnit
    {
        ...
        public static ProtocolDataUnit ReadFrom(byte[] buffer)
        {
            ...
            // build header data
#if TRACE
            Messages.TraceHex("magic [" + HandshakeHeader.MAGIC_HEADER.ToString("X") + "] dump", headerData, 2);
#endif
            ...
            if (magicHeader == HandshakeHeader.MAGIC_HEADER)
            {
                ...
                Messages.WriteError("header magic expected [" + HandshakeHeader.MAGIC_HEADER.ToString("X") + "] got [" + magicHeader.ToString("X") + "]!");
